...
class TD3Engine():
    ...
    def update_step_bptt(self, 
        ...
        rew_t = reward[:, burn_in:]
        terminated_t = terminated[:, burn_in:]
        truncated_t = truncated[:, burn_in:]
        done_boot = (terminated_t > 0.5).float()
        done_any  = ((terminated_t > 0.5) | (truncated_t > 0.5)).float()


            # ---- n-step: elegir T_eff = T - n_step (mínimo 1)
        T = rew_t.shape[1]
        n = min(int(n_step), max(1, T - 1))   # asegura que exista t+n
        T_eff = T - n

        # Recortar todo a (B, T_eff, ...)
        obs_eff = obs_t[:, :T_eff]
        act_eff = act_t[:, :T_eff]

        # 3) Critic target con n-step
        with torch.no_grad():
            next_act_all, _ = self.actor_target.forward_bptt(next_obs_t, (h_E_t, h_O_t))  # (B,T,act_dim)
            noise = (torch.randn_like(next_act_all) * self.target_policy_noise).clamp(-self.target_noise_clip, self.target_noise_clip)
            next_act_all = (next_act_all + noise).clamp(self.action_low, self.action_high)

            q1_all, q2_all = self.critic_target(next_obs_t, next_act_all)   # (B,T,1)
            min_q_all = torch.min(q1_all, q2_all)

            # bootstrap en t+n
            min_q_n = min_q_all[:, n:n+T_eff]  # (B,T_eff,1)

            discounts = (self.gamma ** torch.arange(n, device=self.device, dtype=torch.float32)).view(1, 1, n)

            r_win = torch.stack([rew_t[:, i:i+T_eff] for i in range(n)], dim=2)      # (B,T_eff,n)
            d_win = torch.stack([done_any[:, i:i+T_eff] for i in range(n)], dim=2)     # (B,T_eff,n)

            alive = torch.cumprod(torch.cat([
                torch.ones((B, T_eff, 1), device=self.device),
                (1.0 - d_win[:, :, :-1])
            ], dim=2), dim=2)  # (B,T_eff,n)

            G = (alive * discounts * r_win).sum(dim=2)  # (B,T_eff)

            no_done_n_boot = torch.prod(1.0 - torch.stack([done_boot[:, i:i+T_eff] for i in range(n)], dim=2), dim=2)

            target_q = G.unsqueeze(-1) + (no_done_n_boot.unsqueeze(-1) * (self.gamma ** n) * min_q_n)

        q1, q2 = self.critic(obs_eff, act_eff)        
        ...
        if self.total_updates % self.policy_delay == 0:
            ...
            pi_act, _ = self.actor.forward_bptt(obs_eff, (h_E_detached, h_O_detached))
            actor_loss = -self.critic.q1_forward(obs_eff, pi_act).mean()
...
def td3_train(
    ...
    while total_steps < max_train_steps:
        # New episode
        obs, _ = env.reset(seed=env_seed + e)
        ep_reward = 0.0
        ep_actions = []
        steps = 0
        ...
        while not done:
            ...
            if total_steps > warmup_steps and (total_steps % update_every_steps == 0):
                for _ in range(num_update_loops):
                    ...
                    actor_loss, critic_loss = engine.update_step_bptt(seq_batch, burn_in_length, n_step=config.n_step)
