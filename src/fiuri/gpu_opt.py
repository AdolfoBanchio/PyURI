...
class PyUriTwc(nn.Module):
    ...
    def _physics_step(self, state_E, state_O_hybrid):
        ...
        #direction = torch.tanh(diff) # returns -1, 0, or 1
    ...
    def forward(self, obs):
        ...
        if self.stored_E is None or self.stored_E.shape[0] != obs.shape[0]:
            # Resetear usando el dispositivo dinámico
        ...
        # Guardar estados desvinculados del grafo de computación (detach)
    ...
    def forward_bptt(self, obs_sequence, initial_state=None):
        ...
        # 2. Inicializar estados en el dispositivo correcto
        if initial_state is None:
            E, O = self.get_initial_state(B, device)
        else:
            # Asegurar que el estado pasado también esté en el device correcto
            E, O = initial_state[0].to(device), initial_state[1].to(device)

        actions_list = []
        for t in range(T):
            obs_t = obs_sequence[:, t, :]
            # El paso forward_step ya es funcional y mantendrá el device
            action_t, (new_E, new_O) = self.fast_forward_step(obs_t, E, O)
            E = new_E.clone()
            O = new_O.clone()
            actions_list.append(action_t.clone())

        # 3. Stackear resultados (esto heredará el device de los elementos)
...
class PyUriTwc_V2(nn.Module):
    ...
    def forward_bptt(self, obs_sequence, initial_state=None):
        obs_sequence = obs_sequence.to(self.weights.device, non_blocking=True)
        B, T, _ = obs_sequence.shape
        if initial_state is None:
            E, O = self.get_initial_state(B)
        else:
            E, O = initial_state
...
def build_fiuri_twc_v2(steepness_fire: float = 14.0,
